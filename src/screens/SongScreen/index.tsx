...
export default function SongScreen(props: any) {
  ...
  const [state, setState] = useState({
    ...
    song: { groupname: props.route.params.groupname, title: props.route.params.title, text: props.route.params.text, html: '' }
  })
 ...
 */
  const transposeHTML = (_: any, chord: string) => {
    return '<span class="chord">' + transpose(chord, state.transposition) + '</span>';
  }
  ...
  const makeHTML = () => {
    ...
    const bodyHTML = state.song.text.replace(/(.+)/g, '<div class="row">$1&nbsp;</div>')
    ...
    const resultHTML = '<meta name="viewport" content="width=device-width, initial-scale=1">'
      + styleHTML + (state.darkMode ? styleHTMLDark : '') + '<div class="container">' + bodyHTML + '</div>';
    ...
    <View style={[styles.container, { backgroundColor: state.darkMode ? '#000' : '#fff' }]}>
      <View style={styles.songHeader}>
        ...
        <View style={styles.songHeaderTitle}>
          <Text style={[styles.title, { color: state.darkMode ? '#fff' : '#000' }]}>{state.song.title}</Text>
          <Text style={[styles.subtitle, { color: state.darkMode ? '#fff' : '#000' }]}>{state.song.groupname}</Text>
        </View>
        <View style={styles.songHeaderTransposition}>
          ...
          <View><Text style={[styles.shift, { color: state.darkMode ? '#fff' : '#000' }]}>{state.transposition ? (state.transposition < 0 ? state.transposition : "+" + state.transposition) : null}</Text></View>
