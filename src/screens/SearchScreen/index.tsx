...
export default function SearchScreen(props: {
  navigation: {
    navigate: (
      arg0: string,
      arg1: {groupname: any; title: any; text: any; darkMode: boolean},
    ) => void;
  };
}) {
  ...
  const createDb = async () => {
    try {
      await populateDb((status: string) => {
        setState({...state, appState: status});
      });
    } catch (error) {
  ...
  const updateSongList = async (query: string) => {
    if (state.dbExists) {
      setState({...state, songList: await getSongList(query)});
    }
  };
  ...
  const renderPopulateDbButton = () => {
    return (
      <TouchableOpacity onPress={() => createDb()}>
        ...
        />
        <Text
          style={[styles.loadDb, {color: state.darkMode ? '#fff' : '#000'}]}>
          Nahrát zpěvník{'\n'} z internetu (13 MB)
        </Text>
      </TouchableOpacity>
    );
      ...
      <MenuProvider>
            ...
            <MenuOptions>
              <MenuOption onSelect={() => toggleDarkMode()}>
                <Text style={styles.menuItem}>
                  {state.darkMode ? 'Zrušit ' : ''}Noční režim
                </Text>
              </MenuOption>
              <MenuOption onSelect={() => toggleAwake()}>
                <Text style={styles.menuItem}>
                  {state.keepAwake ? 'Zhasínat ' : 'Nezhasínat '}displej
                </Text>
              </MenuOption>
            </MenuOptions>
          </Menu>
        ...
        {state.appState === '' && state.dbExists && !state.songList.length ? (
          <>
            ...
            <View style={styles.imageGiraffe}>
              <GiraffeIcon width="250" height="250" />
            </View>
          </>
        ) : null}
      </MenuProvider>
