...
export async function existsDb(): Promise<boolean> {
  ...
  try {
    const results = await db.executeSql(`SELECT name FROM sqlite_master WHERE type='table' AND name='zpevnikator';`)
...
export async function purgeDb() {
  ...
  const db = await getDBConnection();
  try {
    const results = await db.executeSql('DROP TABLE zpevnikator;')
  } catch (error) {
    console.error(error);
    return false
  }
...
async function createDb() {
  const db = await getDBConnection();
  try {
    const results = await db.executeSql('CREATE VIRTUAL TABLE IF NOT EXISTS zpevnikator USING fts4(id INTEGER PRIMARY KEY AUTOINCREMENT, groupname TEXT, title TEXT, text TEXT, textclean TEXT, tokenize=unicode61);')
  } catch (error) {
    console.error(error);
    return false
  }
...
export async function getSongList(query: string) {
  ...
  await db.transaction(async tx => {
    ...
    var [tx, results] = await tx.executeSql('SELECT groupname,title,text,snippet(zpevnikator, "<b>", "</b>","...", 4, 10) AS snippet FROM zpevnikator WHERE title MATCH ? OR groupname MATCH ? OR zpevnikator MATCH ? LIMIT 42', [query, query, query])
